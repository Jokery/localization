...
// ▼リソース読込
	...
	// エフェクト
	LoadEffect_("ef881_05_door_b");		// 扉の光 青
...
//-------------------------------------------------------------------------
	...
	SetMotion_(green_door01, MOT_GIMMICK_1, BLEND_XL);							// １つ目扉が開く
	PlaySE_("SE_DEM_196");
	SetMotionSpeed_(green_door01, 0.6);
	Wait_(45);
	efc_door000 = SetPointWorldEffect_("ef881_05_door_b", "efc_door000");
	SetMotionSpeed_(green_door01, 0.3);
	Wait_(35);
	PlaySE_("SE_DEM_197");
	Wait_(70);
	SetMotionSpeed_(green_door01, 0.4);
	WaitMotion(green_door01);
	Wait_(15);
	//=====================
	//二つ目の扉が開く
	//=====================
	SetMotion_(right_door, MOT_GIMMICK_1, BLEND_M);								// 2つ目扉が開く
	PlaySE_("SE_DEM_197");
	SetMotionSpeed_(right_door, 0.45);
	WaitMotion(right_door);
	...
	//===============================================
	// ■アロマ
	// 「時空の裂け目の封印を　解除したわ。　これで海の神ワダツミと　戦えるわよ。
	//-----------------------------------------------
	OpenMsgWnd_();
	SetTalkName_("NAME_TAG_AROMA");
	ShowMsg_("DEMO_811_MSG_010");
	KeyInputWait_();
	//「アンタの勝利を　祈ってるわ。　がんばんなさいよ　<name_player/>！
	ShowMsg_("DEMO_811_MSG_020");
	KeyInputWait_();
	CloseMsgWnd_();
	//-----------------------------------------------
	Wait_(30);
