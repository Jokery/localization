...
// ▼フラグ設定
	local bit_battery01 = GetEventFlg_("BFG_BATTERY_FIRST");		// Bit_1044 蓄電器①起動

	if (bit_battery01) {
		gimmick_motion = MOT_GIMMICK_1;		// 白
		msg_key00 = "DEMO_105_MSG_030"		// 「よしよし。もう１個の　サブ電源もあったな。
	}
	else {
		gimmick_motion = MOT_GIMMICK_0;		// 赤
		msg_key00 = "DEMO_105_MSG_010"		// 「そいつが　サブ電源だぜ
	}
...
// ▼配置
	...
	SetAlpha_(light00, ALPHA_CLEAR);
	// ライト（右）予備
	local light_spare = ArrangePointGimmick_("o_A02_04", model_light_R, "obj_lightR000");
	...
	SetMotion_(monitor, MOT_GIMMICK_2, BLEND_N);						// モニタ(蓄電器左点灯)
...
//-------------------------------------------------------------------------
	...
	SetMotion_(storage01, MOT_GIMMICK_1, BLEND_M);		// 電源ON
	Wait_(30);
	...
	SetMotion_(light00, MOT_GIMMICK_1, BLEND_M);		// 赤→白
	for (local i=1; i<=10; i++) {
		SetAlpha_(light00, ALPHA_CLEAR + (i * 0.1));
		SetAlpha_(light_spare, ALPHA_EXIST - (i * 0.1));
		Wait_(1);
	}
	SetMotion_(light00, MOT_GIMMICK_1, BLEND_N);
