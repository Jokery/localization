...
function FadeIn()
{
	...
	local status_num = GetNumFlg_("NUM_YATIHOKO_PHASE");
	DebugPrint(" ");
	DebugPrint("NUM_YATIHOKO_PHASE の値は " + status_num + " になりました。");
...
function EventStartOperate()
{
	...
	// 勝敗判定
		...
		if(GetBattleEndType_() == END_TYPE_WIN){
			...
			SetEventFlg_("BFG_GET_LAND_STAR_PIECE", true); // 陸のスターピース取得フラグ
			SetEventFlg_("BFG_KNOCK_DOWN_YATIHOKO", true); // ヤチホコ撃破取得フラグ
			// 復帰位置
			SetReturnDemoPosDir_(Vec3(RTN_BATTLE_POS_X, RTN_BATTLE_POS_Y, RTN_BATTLE_POS_Z), RTN_BATTLE_DIR);
			// アイテム取得(Redmine#2349)
			AddItem_(LAND_STAR_PIECE_ID, 1);
			// Demo814[陸のスターピース入手]再生
