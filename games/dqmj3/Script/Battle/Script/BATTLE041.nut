...
function Init()
{
	DebugPrint("BATTLE041_IN_function_Init")
	BattleCommon_Init();
	...
	DebugPrint("BATTLE041_OUT_function_Init")
	LoadEffect();
...
function Update()
{
	DebugPrint("BATTLE041_function_Update")
	...
	if(gAttackNum == 4){
		...
		S2Action();					//Ｓ２体構成
	} else if(gAttackNum == 2 && gSFlag == true && gMFlag == true ){
		S1M1Action();				//Ｓ１体＋Ｍ１体構成
	} else if(gAttackNum == 2 && gSFlag == false && gMFlag == true ){
		M2Action();					//Ｍ２体構成
	} else{
		//ＧorＴ１体構成
	}
...
{
	DebugPrint("BATTLE041_function_S4Action")
	// アニメーション動作セット
	...
	Wait_(23);
	gAngle = -135;
	local effect= SetBoneEffect_("EF000_14_SMOKE_FOOT", gTarget);
	SetDamageMotion_Rush(gAttack1,gTarget,gAngle,gMultiCount,gMultistagecount,effect);
	...
	if(DeadCheck == true){
		...
		gAngle = 135;
		SetDamageMotion_Rush(gAttack2,gTarget,gAngle,gMultiCount,gMultistagecount,effect);
		...
		if(IsDead_(gMultiCount,0)){
			...
			gAngle = -135;
			SetDamageMotion_Rush(gAttack3,gTarget,gAngle,gMultiCount,gMultistagecount,effect);
			...
			if(IsDead_(gMultiCount,0)){
				...
				Wait_(10 - GetActHitNum_(gMultiCount-1,0));
				gAngle = 135;
				SetDamageMotion_Rush(gAttack4,gTarget,gAngle,gMultiCount,gMultistagecount,effect);
				//SetDamageMotion_Rush(gAttack4);
				/*
				if(IsDead_(gMultiCount,0)){
...
{
	DebugPrint("BATTLE041_function_S3Action")
	// アニメーション動作セット
...
{
	DebugPrint("BATTLE041_function_S2M1Action")
	// アニメーション動作セット
...
{
	DebugPrint("BATTLE041_function_S2Action")
	// アニメーション動作セット
...
{
	DebugPrint("BATTLE041_function_S1M1Action")
	// アニメーション動作セット
...
{
	DebugPrint("BATTLE041_function_M2Action")
	// アニメーション動作セット
	Task_AnimeMoveCamera_("camera_005" , "camtgt_005");
...
{
	DebugPrint("BATTLE041_function_gTargetDead")
	local deadCnt = 0;
	local cntMax = 30;
