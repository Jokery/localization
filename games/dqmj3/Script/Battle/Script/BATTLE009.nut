...
function Init()
{
	DebugPrint("BATTLE009_function_Init");
	BattleCommon_Init();
...
function Update()
{
	DebugPrint("BATTLE009_function_Update");
...
function SAction()
{
	DebugPrint("BATTLE009_function_SAction");
	local WaitFrame = 7;
	local StartFrame = 151 - WaitFrame;
	
	if(cTarget.Size == SIZE_G || cTarget.Size == SIZE_T){
		WaitFrame = 20;
		StartFrame = 151 - WaitFrame;
	}
	...
	SetMotionFrame_(gAttack,30-(WaitFrame % 30))
...
function MAction()
{
	DebugPrint("BATTLE009_function_MAction");
	local WaitFrame = 20;
	local StartFrame = 151 - WaitFrame;
