...
while ( four = content[index, 4] )
  index += 4
  if [ "\x43\x00\x00\x00", "\x85\x00\x00\x00" ].include?(four)
    ...
    index += length - 8
  elsif [ "\x0c\x04\x00\x00", "\x09\x04\x00\x00", "\x0a\x04\x00\x00", "\x0e\x04\x00\x00" ].include?(four)
...
File.open('repack_MAA21', 'wb') { |f| f.write content }
